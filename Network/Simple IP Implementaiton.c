...
// 해당 routing entry 에는 다른 네트워크로 가기 위한 가장 좋은 라우터의 정보를 가지고 있어서, 효율적인
// 도달을 가능하게 한다
...
int my_ip_output(struct my_ip_header my_iph, struct my_ip_routing_entry * rentry) { 
	...
	if(rentry == NULL){
		memcpy(next_hop, my_iph.dest, sizeof(my_iph.dest) + 1);
	} else {
		memcpy(next_hop, rentry, sizeof(rentry) + 1);
	}
	...
	while(cache_ptr != NULL) {
		...
		next_hop_mac = (unsigned char *)malloc(7);
		memcpy(next_hop_mac, cache_ptr->dest_mac, 7);
		cache_ptr = cache_ptr->next;
	}
